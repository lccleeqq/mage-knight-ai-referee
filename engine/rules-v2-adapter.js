// Adapter: connects the v2 normalizer + structured rules to the live referee.
// It deliberately returns insufficient_information when a rule cannot be proven.
function normalizeActionV2(text){
  ...
  else if(has(/(?:攻擊|attack)[\s\S]*(?:敵人|enemy)/i)){out.action='attack';out.confidence=.9;}
  else if(has(/(?:移動|move)[\s\S]*(?:森林|平原|沙漠|沼澤|forest|plains|desert|swamp)/i)){out.action='move';out.confidence=.85;}
